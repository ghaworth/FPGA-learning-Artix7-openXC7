...
- Key design decisions:
  ...
  - **BRAM inference bug found and fixed upstream**: Yosys initially inferred the count array as RAMB36E1 SDP but a one-character typo in the Xilinx techlib (`PORT_W_WIDTH == 71` should have been `== 72` in `brams_xc6v_map.v` and `brams_xcu_map.v`) silently corrupted data bits 68–69, producing a deterministic wrong answer (8,995 vs 231,229,866,702,355). Worked around with `(* ram_style = "distributed" *)` while investigating, then traced the root cause and submitted [a fix](https://github.com/YosysHQ/yosys/pull/5833) which has been merged. The workaround has since been removed.
